...
import { fetch, deleteItem } from '../../stores/invoices/invoicesSlice';
...
const TableSampleInvoices = ({ filterItems, setFilterItems, filters }) => {
  ...
  const { invoices, loading, count } = useAppSelector(
  ...
  const invoicesPaginated = Array.from(
  ...
  const handleDeleteAction = async () => {
    if (id) {
      ...
      setIsModalTrashActive(false);
      notify('success', 'Invoices was deleted!');
    }
  };
            ...
            invoices.map((item: any) => (
              ...
              >
                ...
                <td data-label='user_id'>
                  {dataFormatter.usersOneListFormatter(item.user_id)}
                </td>

                <td data-label='job_ids'>
                  {dataFormatter.jobsManyListFormatter(item.job_ids).join(', ')}
                </td>
