...
import { fetch, deleteItem } from '../../stores/jobs/jobsSlice';
...
const TableSampleJobs = ({ filterItems, setFilterItems, filters }) => {
  ...
  const { jobs, loading, count } = useAppSelector((state) => state.jobs);
  ...
  const jobsPaginated = Array.from(
  ...
  const handleDeleteAction = async () => {
    if (id) {
      ...
      setIsModalTrashActive(false);
      notify('success', 'Jobs was deleted!');
    }
  };
      ...
      <table>
        ...
        <tbody>
          {jobs &&
            Array.isArray(jobs) &&
            jobs.map((item: any) => (
              <tr key={item.id} onClick={() => router.push(`/jobs/${item.id}`)}>
                ...
                <td data-label='assigned_vehicle_ids'>
                  {dataFormatter
                    .vehiclesManyListFormatter(item.assigned_vehicle_ids)
                    .join(', ')}
                </td>

                <td data-label='assigned_user_id'>
                  {dataFormatter.usersOneListFormatter(item.assigned_user_id)}
                </td>
